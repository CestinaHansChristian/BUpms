<template>
    <div v-for="(item, index) in project_info" :key="index" >
        <ClientOnly>
            <div v-if="item.User === pb.authStore.model?.id" class="project-quick-status grid gap-y-2 py-3 place-content-start lg:flex lg:justify-between border-2 border-slate-300 rounded-md bg-slate-100 mx-5 px-2">
                <div class="project-name grid text-slate-500 font-semibold md:text-lg capitalize lg:w-20">
                    <span class="text-sm text-slate-500 lg:hidden">Title:</span>
                    {{ item.Title }}
                </div>
